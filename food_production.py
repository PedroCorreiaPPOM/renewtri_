...
from utils import current_user_role, metric_card, page_header, turnos
...
def show_food_production(school_id: int) -> None:
    ...
    with left:
        ...
        with st.form("production_form", clear_on_submit=True):
            ...
            desperdicio = st.number_input("Desperdício em kg", min_value=0.0, step=0.1, value=5.0)
            observacoes = st.text_area("Observações", placeholder="Ex.: baixa frequência por chuva")
    ...
    with right:
        ...
        c1, c2 = st.columns(2)
        with c1:
            metric_card("Total de refeições", f"{int(total_meals):,}".replace(",", "."), "Histórico geral")
        with c2:
            metric_card("Desperdício total", f"{total_waste:.1f} kg".replace(".", ","), "Histórico geral")
