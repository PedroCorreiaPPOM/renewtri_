...
def show_food_inventory(school_id: int) -> None:
    ...
    with left:
        ...
        with st.form("inventory_form", clear_on_submit=True):
            ...
            observacoes = st.text_area("Observações", placeholder="Ex.: entrega conferida pela coordenação")
    ...
    with right:
        ...
        c1, c2 = st.columns(2)
        with c1:
            metric_card("Alimentos recebidos", f"{total:.1f} kg".replace(".", ","), "Total no histórico")
        with c2:
            metric_card("Fornecedores", str(suppliers), "Cadastrados no MVP")
